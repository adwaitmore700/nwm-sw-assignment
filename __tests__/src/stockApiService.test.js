...
describe("Execution of main program", () => {
  console.log = jest.fn();
  ...
  it("Should log error if there is no stock selected", async () => {
    ...
    expect(console.log).toHaveBeenCalledWith(RESOURCES.INVALID_DATA);
  });

  it("Should log error if there is external APIs throw error", async () => {
    const testData = [{ ticker: "", quantity: 2 }];
    await getStockMarketPortfolioMock(testData);
    expect(console.log).toHaveBeenCalledWith(RESOURCES.API_ERROR_TEXT);
  });
  ...
  describe("Execution of function getCompanyCurrentPrice", () => {
    ...
    it("Should throw error while making API call", async () => {
      await expect(
